...
public final class WireframeRenderer implements RendererImpl {
    ...
    public void render(GraphicsContext gc, Camera camera, Model model,
        ...
        // создаём WritableImage для буфера кадра
        WritableImage frame = new WritableImage(width, height);

        // PixelWriter для прямого рисования
        var pw = frame.getPixelWriter();
        ...
        // отрисовка модели
        renderModel(gc, model, mvp, zBuffer, width, height, pw);

        // выводим готовый кадр на Canvas
    ...
    private void renderModel(GraphicsContext gc, Model model,
        ...
        for (Polygon polygon : model.getPolygons()) {
            ...
            triangulateAndRasterize(verts, model, zBuffer, width, height, pw);
        }
    }
    ...
    private void rasterizeTriangle(Vertex v0, Vertex v1, Vertex v2,
        ...
        Image tex = model.getTexture(); // текстура конкретной модели
                ...
                if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
                    ...
                    if (z < zBuffer[x][y]) {
                        ...
                        if (tex != null) {
                            ...
                            int tx = (int) Math.clamp(u * (tex.getWidth() - 1), 0, (int)tex.getWidth() - 1);
                            int ty = (int) Math.clamp((1 - v) * (tex.getHeight() - 1), 0, (int)tex.getHeight() - 1);
    ...
    public Model applyTransform(Model originalModel, Transform transform) {
        ...
        // переносим текстуру
        transformed.setTexture(originalModel.getTexture());

        return transformed;
    }

    private Matrix4f createModelMatrix(Transform t) {
        return com.cgvsu.render_engine.GraphicConveyor.createModelMatrix(
